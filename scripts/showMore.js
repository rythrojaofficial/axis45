...
export function populateShowMore() {
  ...
  for (let i = 0; i < showMoreArray.length; i++) {
    ...
    conditionallyVisibleContent.classList.add(
      "show-more-hide-conditionally-visible"
    );
    let showHideButton = new ButtonElement(
      showMoreArray[i],
      showHideMore,
      {},
      options.showMore
    ...
    function showHideMore() {
      switch (isShowMore) {
        case false:
          conditionallyVisibleContent.classList.remove(
            "show-more-hide-conditionally-visible"
          );
