    ...
    switch (inputObject.type){
        ...
        case 'select':
            ...
            // let newOption = new HtmlElement('option', newSelect.element, {}, 'blablah')
                inputObject.appendedOptions.forEach((option)=>{
                    ...
                    newSelect.element, {value: option}, option)
                }
                )
            break;
        case 'radio':
            ...
            let newRadioDiv = new HtmlElement('div', newRadio.element)
                switch(option){
                    case 'other':
                        ...
                        break;
                    default:
                        let newRadioOption = new HtmlElement('input', newRadioDiv.element, 
                            { 
                                type: 'radio'
                            ...
                            if(inputObject.required){newRadioOption.element.setAttribute('required', true)}
                        let newRadioLabel = new HtmlElement('label', newRadioDiv.element,
                            { for: newRadioOption.element.id}, option);
                        break;
                }
        ...
        case 'checkbox':
            ...
            inputObject.appendedOptions.forEach((option)=>{
                ...
                let newCheckboxOption = new HtmlElement('input', newCheckboxDiv.element, 
                { 
                    type: 'checkbox'
                    ,id: `option ${theName} ${option}`
                    ,name: theName
                ...
                if(inputObject.required){newCheckboxOption.element.setAttribute('required', true)}
                let newCheckboxLabel = new HtmlElement('label', newCheckboxDiv.element,
                { for: newCheckboxOption.element.id}, option);
                })
...
function validateForm(formID){
    let requiredArray = Array.from(document.getElementById(formID).querySelectorAll('[required = true]'))
    let counter = 0;
